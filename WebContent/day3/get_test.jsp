...
<body>
	...
	<%
		for(int i=0; i<list.size();i++){
			...
			<!-- 조회를 위해서 get 방식의 요청을 보냅니다. 1) 요청 url에 쿼리스트림으로 조회할 값을 전달합니다.
					url에 ? 기호 뒤에 key=value 한쌍으로 값을 전달합니다.
					전달하는 url에 현재 url과 같으면 생략할 수 있습니다 
					a태그의 href속성 또는 자바스크립트의 location.href='(url)'로 사용합니다.-->
	...
	%>
			...
			<!-- 2) 검색값을 입력받아 조회할 때에는 form 사용합니다.
				실행된 url : http://localhost:8081/jsp_1/day3/get_result.jsp?custno=100002
			 -->
			<hr>
			<form action="get_result.jsp"> <!-- method='get'생략 -->
				<input name="custno" placeholder="회원번호입력"> <button>전송</button>
